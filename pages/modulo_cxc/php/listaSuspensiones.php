    ...
    if($_POST["susCobrador"]!="todos"){
      ...
      if ($_POST["susServicio"] == "C") {
        ...
        if($_POST["susCobrador"]=="todos"){//cable y todos los cobradores
          $query = "SELECT codigoCliente, codigoCobrador, nombre, direccion, tipoServicio,'C' as filtro, COUNT(*) as cantidadDeFacturasVencidas, fechaVencimiento, SUM(cuotaCable + totalImpuesto) as totalDeuda
    ...
    UNION
      ...
      $servicioReporte="Internet";
        if($_POST["susCobrador"]=="todos"){//internet y todos los cobradores
          $query = "SELECT codigoCliente, codigoCobrador, nombre, direccion, tipoServicio,'I' as filtro, COUNT(*) as cantidadDeFacturasVencidas, fechaVencimiento, SUM(cuotaInternet + totalImpuesto) as totalDeuda
    ...
    UNION
      ...
      $servicioReporte="Paquete";
       if($_POST["susCobrador"]=="todos"){//paquete y todos los cobradores
          $query = "SELECT codigoCliente, codigoCobrador, nombre, direccion, tipoServicio,'P' as filtro, COUNT(*) as cantidadDeFacturasVencidas, fechaVencimiento, SUM(cuotaCable + cuotaInternet + totalImpuesto) as totalDeuda
FROM tbl_cargos WHERE estado ='pendiente' AND anulada=0 /* AND   tipoServicio='C' AND tipoServicio='I' */ AND  fechaVencimiento < DATE(NOW())
AND codigoCliente NOT IN (
            /*---SOLO CABLE TODOS LOS COBRADORES con >= 2 facturas vencidas*/
				SELECT codigoCliente FROM tbl_cargos WHERE estado ='pendiente' AND anulada=0 AND tipoServicio='C' AND fechaVencimiento < DATE(NOW())
				AND codigoCliente NOT IN (

				/* ya esta todo como antes*/
					/*--Internet en general*/
					SELECT codigoCliente FROM tbl_cargos WHERE estado ='pendiente' AND anulada=0 AND tipoServicio='I' AND fechaVencimiento < DATE(NOW()) group by `codigoCliente` HAVING COUNT(*) >= 2
					/*--fin Internet en general*/
				) group by `codigoCliente` HAVING COUNT(*) >=2
			...
			/*---SOLO INTERNET TODOS LOS COBRADORES con >= 1 facturas vencidas*/
				...
				AND codigoCliente NOT IN (
					/*--Cable en general*/
					SELECT codigoCliente FROM tbl_cargos WHERE estado ='pendiente' AND anulada=0 AND tipoServicio='C' AND fechaVencimiento < DATE(NOW()) group by `codigoCliente` HAVING COUNT(*) >= 2
					/*--fin Cable en general*/
				) group by `codigoCliente` HAVING COUNT(*) >= 2
...
AND codigoCliente NOT IN (
			...
			/*---FIN SOLO CABLE TODOS LOS COBRADORES con >= 2 facturas vencidas*/
              UNION
			/*---SOLO INTERNET TODOS LOS COBRADORES con >= 2 facturas vencidas*/
				SELECT codigoCliente FROM tbl_cargos WHERE estado ='pendiente' AND anulada=0 AND tipoServicio='I' AND fechaVencimiento < DATE(NOW())
				AND codigoCliente NOT IN (
					/*--Cable en general*/
					SELECT codigoCliente FROM tbl_cargos WHERE estado ='pendiente' AND anulada=0 AND tipoServicio='C' AND fechaVencimiento < DATE(NOW()) group by `codigoCliente` HAVING COUNT(*) >= 2
			...
			/*---FIN SOLO INTERNET TODOS LOS COBRADORES con >= 2 facturas vencidas*/
            UNION
					/**Todos Los Clientes Suspendidos**/
					/**clientes de solo cable ya suspendidos*/
					SELECT cod_cliente FROM clientes WHERE servicio_suspendido='T' AND sin_servicio='F' AND estado_cliente_in = 3
...
FROM tbl_cargos WHERE estado ='pendiente' AND anulada=0 AND fechaVencimiento < DATE(NOW()) AND codigoCobrador=".$_POST["susCobrador"]."
            AND codigoCliente NOT IN (
              /*---SOLO CABLE POR COBRADOR ESPECIFICO CON >=2 facturas vencidas*/
					SELECT codigoCliente FROM tbl_cargos WHERE estado ='pendiente' AND anulada=0 AND tipoServicio='C' AND fechaVencimiento < DATE(NOW()) AND codigoCobrador=".$_POST["susCobrador"]."
					AND codigoCliente NOT IN (
						/*--Internet en general*/
						SELECT codigoCliente FROM tbl_cargos WHERE estado ='pendiente' AND anulada=0 AND tipoServicio='I' AND fechaVencimiento < DATE(NOW()) AND codigoCobrador=".$_POST["susCobrador"]."
              ...
              /*---FIN SOLO CABLE POR COBRADOR ESPECIFICO CON >=2 facturas vencidas*/
              UNION
...
SELECT codigoCliente FROM tbl_cargos WHERE estado ='pendiente' AND anulada=0 AND fechaVencimiento < DATE(NOW()) AND codigoCobrador=".$_POST["susCobrador"]."
    ...
    /*fin de todos los clientes que tienen los dos servicios y ya estan vencidas las 2 facturas*/
)  group by `codigoCliente` HAVING COUNT(*) >= 2) as facturasCable;";
      $resultadoCableDelPaquete= $mysqliCableDePaquete->query($queryCableDelPaquete) ;
      while($row = $resultadoCableDelPaquete->fetch_assoc()){
        $totalCantidadDefacturasCableDePaquete=$row['cantidadFacturasCableDePaquete'];
  ...
  SELECT COUNT(*) as cantidadDeFacturasVencidas , SUM(cuotaInternet + totalImpuesto) as totalDeuda FROM tbl_cargos WHERE estado ='pendiente' AND anulada=0 AND tipoServicio='I' AND fechaVencimiento < DATE(NOW()) AND codigoCobrador=".$_POST["susCobrador"]."
    ...
    /*fin de todos los clientes que tienen los dos servicios y ya estan vencidas  2 facturas*/
)  group by `codigoCliente` HAVING COUNT(*) >= 2) as facturasInternet;";
      $resultadoInternetDelPaquete= $mysqliInternetDePaquete->query($queryInternetDelPaquete) ;
      while($row = $resultadoInternetDelPaquete->fetch_assoc()){
        $totalCantidadDefacturasInterDePaquete = $row['cantidadFacturasInternetDePaquete'];
...
$pdf->Ln(1);
      ...
      $pdf->Cell(120,6,utf8_decode('Direccion'),1,0,'L');
      $pdf->Cell(22,6,utf8_decode('Tipo Servicio'),1,0,'L');
      $pdf->Cell(30,6,utf8_decode('Cant. Fact. Vencidas'),1,0,'L');
      ...
      $pdf->Ln(6);
        ...
        $cell_width = 125;
        $pdf->SetFont('Arial','',5);
        $pdf->MultiCell(120,3,utf8_decode($row['direccion']),0,'J',0);
        $pdf->SetXY($current_x + $cell_width, $current_y);
       $pdf->SetFont('Arial','',5);

        
                //$pdf->Cell(92,3,utf8_decode($row['direccion']),0,0,'L');
                $pdf->Cell(20,3,utf8_decode($tipoServicioTemp.'-'.$row['codigoCobrador']),0,0,'L');
                $pdf->Cell(30,3,utf8_decode($row['cantidadDeFacturasVencidas']),0,0,'C');
        ...
        //TOTAL Ventas
        $pdf->Cell(214,5,utf8_decode('TOTAL: '),0,0,'R');
        $pdf->Cell(30,5,$totalCantidadDeFacturasReporte,"T",0,'C');
