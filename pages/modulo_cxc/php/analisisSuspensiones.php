...
//Verificamos el período de tiempo de suspensiones a analizar
$desde = $_POST["desde"];
$hasta = $_POST["hasta"];
...
function facturasGeneradasDosMesesOrdenadaPorColonias(){
    ...
    global $colonia, $iva, $cesc, $desde, $hasta, $tipoAnalisis;
    ...
    $pdf->SetFont('Arial','',8);
    if ($tipoAnalisis == "su"){
        $pdf->Cell(260,6,utf8_decode("INFORME GENERAL DE CLIENTES SUSPENDIDOS."),0,1,'C');
    }elseif ($tipoAnalisis == "in"){
        $pdf->Cell(260,6,utf8_decode("INFORME GENERAL DE NUEVAS INSTALACIONES."),0,1,'C');
    }else{
        $pdf->Cell(260,6,utf8_decode("INFORME GENERAL DE NUEVAS RENOVACIONES."),0,1,'C');
    }
    $pdf->SetFont('Arial','',6);
    $pdf->Ln(5);
                ...
                switch ($_POST["clServicio"]) {
                    case "C":
                        if ($tipoAnalisis == "su"){
                            ...
                            from clientes WHERE (servicio_suspendido='T') AND sin_servicio='F' AND estado_cliente_in = 3 AND cod_cliente<>'00000' AND fecha_suspencion BETWEEN '{$desde}' AND '{$hasta}'
                            /*fin clientes solo cable activos*/";
                        }elseif ($tipoAnalisis == "in"){
                            $servicioReporte="CABLE";
                            $FiltroServicio="/*clientes solo cable activos*/
                            select cod_cliente, nombre, direccion_cobro, id_colonia, dia_cobro,'Cable' as servicio, telefonos, fecha_ult_pago, cod_cobrador,valor_cuota,cuota_in
                            from clientes WHERE (servicio_suspendido='F' OR servicio_suspendido is null) AND sin_servicio='F' AND estado_cliente_in = 3 AND cod_cliente<>'00000' AND fecha_instalacion BETWEEN '{$desde}' AND '{$hasta}'
                            /*fin clientes solo cable activos*/";
                        }else{
                            $servicioReporte="CABLE";
                            $FiltroServicio="/*clientes solo cable activos*/
                            select cod_cliente, nombre, direccion_cobro, id_colonia, dia_cobro,'Cable' as servicio, telefonos, fecha_ult_pago, cod_cobrador,valor_cuota,cuota_in
                            from clientes WHERE (servicio_suspendido='F' OR servicio_suspendido is null) AND sin_servicio='F' AND estado_cliente_in = 3 AND cod_cliente<>'00000' AND ult_ren_in BETWEEN '{$desde}' AND '{$hasta}'
                            /*fin clientes solo cable activos*/";
                        }
                        break;
                    case "I":
                        if ($tipoAnalisis == "su"){
                            ...
                            from clientes WHERE (servicio_suspendido='F' or servicio_suspendido is null or servicio_suspendido='') and sin_servicio='T' AND estado_cliente_in = 2 AND cod_cliente<>'00000' AND fecha_suspencion_in BETWEEN '{$desde}' AND '{$hasta}'
                            /*fin clientes solo Internet activos */";
                        }
                        elseif ($tipoAnalisis == "in"){
                            $servicioReporte="INTERNET";
                            $FiltroServicio="/*clientes solo Internet activos */
                            select cod_cliente, nombre, direccion_cobro, id_colonia, (dia_corbo_in) as dia_cobro,'Internet' as servicio, telefonos, fecha_ult_pago, cod_cobrador,valor_cuota,cuota_in
                            from clientes WHERE (servicio_suspendido='F' or servicio_suspendido is null or servicio_suspendido='') and sin_servicio='T' AND estado_cliente_in = 1 AND cod_cliente<>'00000' AND fecha_instalacion_in BETWEEN '{$desde}' AND '{$hasta}'
                            /*fin clientes solo Internet activos */";
                        }else{
                            $servicioReporte="INTERNET";
                            $FiltroServicio="/*clientes solo Internet activos */
                            select cod_cliente, nombre, direccion_cobro, id_colonia, (dia_corbo_in) as dia_cobro,'Internet' as servicio, telefonos, fecha_ult_pago, cod_cobrador,valor_cuota,cuota_in
                            from clientes WHERE (servicio_suspendido='F' or servicio_suspendido is null or servicio_suspendido='') and sin_servicio='T' AND estado_cliente_in = 1 AND cod_cliente<>'00000' AND ult_ren_in BETWEEN '{$desde}' AND '{$hasta}'
                            /*fin clientes solo Internet activos */";
                        }
                        break;
                    case "A":
                        if ($tipoAnalisis == "su"){
                            ...
                            from clientes WHERE (servicio_suspendido='T') AND sin_servicio='F' AND estado_cliente_in = 2 AND cod_cliente<>'00000' AND (fecha_suspencion_in BETWEEN '{$desde}' AND '{$hasta}') OR (fecha_suspencion BETWEEN '{$desde}' AND '{$hasta}')
                            /*fin clientes paquete activos*/";
                        }elseif ($tipoAnalisis == "in"){
                            $servicioReporte="PAQUETE";
                            $FiltroServicio="/*clientes paquete activos*/
                            select cod_cliente, nombre, direccion_cobro, id_colonia, dia_cobro,'Paquete' as servicio, telefonos, fecha_ult_pago, cod_cobrador,valor_cuota,cuota_in
                            from clientes WHERE (servicio_suspendido='F' or servicio_suspendido is null or servicio_suspendido='') and sin_servicio='F' AND estado_cliente_in = 1 AND cod_cliente<>'00000' AND (fecha_instalacion_in BETWEEN '{$desde}' AND '{$hasta}') OR (fecha_instalacion BETWEEN '{$desde}' AND '{$hasta}')
                            /*fin clientes paquete activos*/";
                        }else{
                            $servicioReporte="PAQUETE";
                            $FiltroServicio="/*clientes paquete activos*/
                            select cod_cliente, nombre, direccion_cobro, id_colonia, dia_cobro,'Paquete' as servicio, telefonos, fecha_ult_pago, cod_cobrador,valor_cuota,cuota_in
                            from clientes WHERE (servicio_suspendido='F' or servicio_suspendido is null or servicio_suspendido='') and sin_servicio='F' AND estado_cliente_in = 1 AND cod_cliente<>'00000' AND ult_ren_in BETWEEN '{$desde}' AND '{$hasta}'
                            /*fin clientes paquete activos*/";
                        }
                        break;
                    case "T":
